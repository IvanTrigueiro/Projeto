...
Graph::Graph(){
  adjacent_matrix = ReadFile("inputs/myciel3.col", &numero_vertices);
  vertices = new Vertex[numero_vertices];
...
void Graph::grasp()
{
  std::vector<Vertex> melhorsolucao;
  ...
  for( int p = 0; p < 50; p++ )
  {
    if(melhorsolucao.empty()){
      bsolution=INT_MAX;
    }
    else{
          bsolution= count_colors_grasp(melhorsolucao);
    }
    Vertex* basesolution= grasp_constructor();
    std::vector<Vertex> solucaobase_;
    for( int i = 0; i < numero_vertices; ++i )
      solucaobase_.push_back( basesolution[i] );

    int solution = count_colors_grasp( solucaobase_ );
    ...
    for( int l = 0; l < numero_vertices; ++l )
      if( !( std::find( actualcolors.begin(), actualcolors.end(), basesolution[l].color ) != actualcolors.end() ) )
        actualcolors.push_back( solucaobase_[l].color );
    ...
    for( int j = 0; j < numero_vertices; ++j )
    {
      if( solucaobase_[j].color == i )
      {
        for( int k = 0; k < solution; ++k )
        {
          if( k != i && ( std::find( actualcolors.begin(), actualcolors.end(), k ) != actualcolors.end() ) )
          {
            if( !search_color_adjacent_grasp( solucaobase_[j].id, k, solucaobase_ ) )
            {
              solucaobase_[j].color = k;
              break;
            }
          }
        }
    ...
    for( int l = 0 ; l < numero_vertices; ++l )
    {
      if( !( std::find( actualcolors.begin(), actualcolors.end(), solucaobase_[l].color ) != actualcolors.end() ) )
      {
        actualcolors.push_back( solucaobase_[l].color );
      }
    }
    int color_count = count_colors_grasp( solucaobase_ );
    //std::cout <<color_count<<" "<<count_colors_grasp(melhorsolucao)<< std::endl;
    if( color_count < bsolution ){
          melhorsolucao=solucaobase_;
    }

    std::cout<<count_colors_grasp(melhorsolucao)<< std::endl;
  }
  std::cout<<count_colors_grasp(melhorsolucao)<< std::endl;
  if(check_colorability(melhorsolucao))
...
Vertex* Graph::grasp_constructor(){
  ...
  Vertex* solucao = new Vertex[numero_vertices];
  for(int i=0;i<numero_vertices;i++){
